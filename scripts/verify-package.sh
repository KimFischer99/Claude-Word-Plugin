...
APPLEDOUBLE_COUNT="$(printf '%s\n' "$PAYLOAD_LIST" | grep -F "/._" | wc -l | tr -d '[:space:]')"
if [ "$APPLEDOUBLE_COUNT" != "0" ]; then
  echo "WARN: package payload contains ${APPLEDOUBLE_COUNT} AppleDouble metadata entries generated by pkgbuild"
fi
